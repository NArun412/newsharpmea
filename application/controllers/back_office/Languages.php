...
check_permission('languages','view');	
if($order == "")
$order ="sort_order";
...
$url_parm_q = "?";
$filters = $this->fct->get_module_fields("languages",FALSE,TRUE);
...
// end filtration
// pagination  start :
$count_news = $this->languages_m->getall($cond,$like);
...
$data['info'] = $this->languages_m->getall($cond,$like,$config['per_page'],$page);
$this->session->set_userdata("admin_redirect_link",$url);
// end pagination .
$this->template->add_js('assets/js/jquery.tablednd_0_5.js');
$this->template->add_js('assets/js/custom/languages.js');
...
$this->template->write_view('content','back_office/languages/list',$data);
...
check_permission('languages','edit',$id);
$data["title"]="Edit languages";
$data["content"]="back_office/languages/add";
$cond=array("id_languages"=>$id);
$data["id"]=$id;
$data["info"]=$this->languages_m->get($id);
$this->template->add_js('assets/js/custom/languages.js');
$this->template->write_view('header','back_office/includes/header',$data);
$this->template->write_view('leftbar','back_office/includes/leftbar',$data);
$this->template->write_view('rightbar','back_office/includes/rightbar',$data);
...
check_permission('languages','delete',$id);
...
redirect(admin_url("languages/".$this->session->userdata("back_link")));
...
if($cehcklist[$i] != ""){
if(has_permission('languages','delete',$cehcklist[$i]))
$this->languages_m->delete($cehcklist[$i]);
}
} } 
...
check_permission('languages','edit',$this->input->post("id"));
else
check_permission('languages','create');
	
$data["title"]="Add / Edit languages";
$this->form_validation->set_rules("title", "TITLE", "trim|required");
if(seo_enabled("languages")) {
...
$_data["lang"] = default_lang();	
	...
	$new_id = $this->languages_m->insert_update($this->input->post(),$this->input->post("id"));
	if(has_permission("languages","edit") && seo_enabled("languages")) {
		$this->fct->insert_update_seo("languages",$new_id,$this->input->post());
	}
