...
$this->load->model(admin_dir()."/users_permissions_m");
$this->template->set_template("admin");
}

public function index($order=""){
check_permission('users_permissions','view');		
if($order == "")
$order ="sort_order";
$data["title"]="List users permissions";
$data["content"]="back_office/users_permissions/list";
//
...
/*$cond = array();
$like = array();
$url = admin_url("users_permissions");
$url_parm_q = "?";
$filters = $this->fct->get_module_fields("users_permissions",FALSE,TRUE);
...
// end filtration
// pagination  start :
$count_news = $this->users_permissions_m->getall($cond,$like);
...
$data['info'] = $this->users_permissions_m->getall($cond,$like,$config['per_page'],$page);
$this->session->set_userdata("admin_redirect_link",$url);*/
// end pagination .
$this->template->add_js('assets/js/jquery.tablednd_0_5.js');
$this->template->add_js('assets/js/custom/users_permissions.js');
...
$this->template->write_view('content','back_office/users_permissions/list',$data);
...
{
	check_permission('users_permissions','edit');	
	...
	$this->db->where("id_users",$user_id)->delete("users_permissions");
	$insert_batch = array();
	...
	if(!empty($all_content_types) && !empty($permissions)) {
		...
		if(!empty($insert_batch)) {
			$this->db->insert_batch("users_permissions",$insert_batch);
			$this->fct->insert_log("update","users_permissions",$user_id);
		}
	}
	$this->session->set_userdata("success_message","Permissions updated successfully.");
	redirect(admin_url("users_permissions").'?id_users='.$user_id);
}
}