...
parent::__construct();
$this->table="routes";
$this->load->model(admin_dir()."/routes_m");
...
$url_parm_q = "?";
$filters = $this->fct->get_module_fields("routes",FALSE,TRUE);
...
// end filtration
// pagination  start :
$count_news = $this->routes_m->getall($cond,$like);
...
$data['info'] = $this->routes_m->getall($cond,$like,$config['per_page'],$page);
$this->session->set_userdata("admin_redirect_link",$url);
// end pagination .
$this->template->add_js('assets/js/jquery.tablednd_0_5.js');
$this->template->add_js('assets/js/custom/routes.js');
...
$this->template->write_view('content','back_office/routes/list',$data);
...
check_permission('routes','edit',$id);
$data["title"]="Edit routes";
$data["content"]="back_office/routes/add";
$cond=array("id_routes"=>$id);
$data["id"]=$id;
$data["info"]=$this->routes_m->get($id);
$this->template->add_js('assets/js/custom/routes.js');
$this->template->write_view('header','back_office/includes/header',$data);
$this->template->write_view('leftbar','back_office/includes/leftbar',$data);
$this->template->write_view('rightbar','back_office/includes/rightbar',$data);
...
check_permission('routes','delete',$id);
$this->routes_m->delete($id);
...
redirect(admin_url("routes/".$this->session->userdata("back_link")));
...
if($cehcklist[$i] != ""){
if(has_permission('routes','delete',$cehcklist[$i]))
$this->routes_m->delete($cehcklist[$i]);
}
} } 
...
check_permission('routes','edit',$this->input->post("id"));
else
check_permission('routes','create');
	
$data["title"]="Add / Edit routes";
$this->form_validation->set_rules("title", "TITLE", "trim|required");
if(seo_enabled("routes")) {
...
$_data["lang"] = default_lang();	
	...
	$new_id = $this->routes_m->insert_update($this->input->post(),$this->input->post("id"));
	if(has_permission("routes","edit") && seo_enabled("routes")) {
		$this->fct->insert_update_seo("routes",$new_id,$this->input->post());
	}
