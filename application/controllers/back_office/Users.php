...
$data["content"]="back_office/users/list";
//
...
$url = admin_url("users");
$url_parm_q = "?";
$filters = $this->fct->get_module_fields("users",FALSE,TRUE);
...
$cond['id_roles >'] = 1;
// pagination  start :
$count_news = $this->users_m->getall($cond,$like);
...
$data['info'] = $this->users_m->getall($cond,$like,$config['per_page'],$page);
$this->session->set_userdata("admin_redirect_link",$url);
// end pagination .
$this->template->add_js('assets/js/jquery.tablednd_0_5.js');
$this->template->add_js('assets/js/custom/users.js');
...
$this->template->write_view('content','back_office/users/list',$data);
...
$data["content"]="back_office/users/add";
$this->template->add_js('assets/js/custom/users.js');
$this->template->write_view('header','back_office/includes/header',$data);
$this->template->write_view('leftbar','back_office/includes/leftbar',$data);
$this->template->write_view('rightbar','back_office/includes/rightbar',$data);
...
check_permission('users','edit',$id);
$data["title"]="Edit users";
$data["content"]="back_office/users/add";
$cond=array("id_users"=>$id);
$data["id"]=$id;
$data["info"]=$this->users_m->get($id);
$this->template->add_js('assets/js/custom/users.js');
$this->template->write_view('header','back_office/includes/header',$data);
$this->template->write_view('leftbar','back_office/includes/leftbar',$data);
$this->template->write_view('rightbar','back_office/includes/rightbar',$data);
...
redirect(admin_url("users/".$this->session->userdata("back_link")));
...
if($cehcklist[$i] != ""){
if(has_permission('users','delete',$cehcklist[$i]))
$this->users_m->delete($cehcklist[$i]);
}
} } 
...
check_permission('users','edit',$this->input->post("id"));
else
check_permission('users','create');
	
$data["title"]="Add / Edit users";
$this->form_validation->set_rules("title", "TITLE", "trim|required");
if(seo_enabled("users")) {
...
$this->form_validation->set_rules("id_divisions", "Division", "trim");
$this->form_validation->set_rules("manage_his_division", "Manage His Division Content Only", "trim");
$this->form_validation->set_rules("manage_his_content", "Manage His Content Only", "trim");
...
$this->form_validation->set_rules("id_roles", "role", "trim|required");
...
if(!empty($_FILES["photo"]["name"])) {
	$_POST["photo"] = $this->fct->upload_image_from_admin("users","photo");
}
...
//$_data["lang"] = default_lang();	
	...
	$new_id = $this->users_m->insert_update($this->input->post(),$this->input->post("id"));
	if(has_permission("users","edit") && seo_enabled("users")) {
		$this->fct->insert_update_seo("users",$new_id,$this->input->post());
	}
