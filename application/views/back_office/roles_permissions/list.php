<?php $content_type = "roles_permissions"; 
...
<?php if($this->input->get("id_roles") != "") {
	$role_id = $this->input->get("id_roles");
	if(!is_numeric($role_id)) exit("access denied");
	$permissions = $this->fct->get_role_permissions($role_id);
	?>
  <div class="row">
    <div class="col-md-12">
      <section class="tile">
...
?>
        <div class="tile-body p-0">
          <div class="table-responsive">
              ...
              <tbody>
                ...
                <tr id="<?php echo $val["id_content"]; ?>">
                  <td class="col-chk" width="10">
                      ...
                      <td width="150"><?php echo ucfirst(str_replace("_"," ",$val['name'])); ?></td>
               	<?php 
							$i=0;
							foreach(permissions() as $perm){ $i++;
							$c = '';
							if(role_has_permission($permissions,$val['used_name'],$perm))
							$c = 'checked="checked"';
							?>
							<td width="10">
                            <?php if($val['perm_'.$perm] == 1) {?>
                            <input <?php echo $c; ?> type="checkbox" class="select-perm-them-all select-perm-them-<?php echo $val["id_content"] ; ?>  select-perm-all-<?php echo $k; ?>" name="perm_<?php echo $perm; ?>[<?php echo $val['id_content']; ?>]" value="1" /><?php }?></td>
						<?php }?>   
                </tr>
                <?php }}?>
              </tbody>
            ...
            <div class="clearfix"></div>
            <div class="row-fluid"><div class="col-md-12"><label class="pull-right"><input type="checkbox" name="update_all" value="1" /> Override all current users permissions</label></div></div>  <div class="clearfix"></div>
            <div class="row"><div class="col-md-12"><input type="submit" class="pull-right btn btn-primary" value="Submit" /></div></div>
          </div>
        </div>
