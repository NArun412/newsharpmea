<?php $content_type = "roles"; ?>
<div class="page page-tables-datatables">
  <div class="pageheader">
    <h2><?php echo $title; ?><span></span></h2>
    <?php if(has_permission($content_type,'create')){ ?>
    <a role="button" href="<?php echo site_url("back_office/".$content_type."/add"); ?>" class="btn btn-success btn-ef btn-ef-3 btn-ef-3c mb-10 pull-right"> <i class="fa fa-plus mr-5"></i> Add <?php echo str_replace("_"," ",$content_type); ?></a>
    <?php } ?>
    <div class="page-bar">
...
echo form_dropdown("status", $options,$selected1, 'class="form-control"');
		  ...
		  foreach($filters as $filter) {
			  ...
			  ?>
                    ...
                    <?php 
					if($filter['type'] == 1) {
						echo render_date($attr_name,$this->input->get($attr_name),$filter['id']);
                    }
					elseif($filter['type'] == 6) {
						echo render_active($attr_name,$this->input->get($attr_name),$filter['id']);
                    }
					elseif($filter['type'] == 7) {
						...
						echo render_textbox($attr_name,$this->input->get($attr_name),$filter['id']);
					}
					 ?>
                    </div>
          <?php }
