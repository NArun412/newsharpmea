...
$tables = getAll("content_type","name");
/*$tables = $this->db->list_tables();
$i = 0;
foreach ($tables as $table)
{
	$i++;
echo "<u><b>".$i.") ".$table. "</b></u>";
echo "<br />";
...
$attributes = array('class' => 'middle-forms');
	$action = admin_url('manage/submit_import/import');
	if(in_array($content_type,static_modules()))
...
if(isset($id)){
	...
	$info["tables"] = $tables[0]["id_content"];
	$info["fields"] = "";
}
?>
<section class="tile">
...
foreach($tables as $key => $table){
    $options[$table["id_content"]] =  str_replace("_", " ", ucfirst($table["name"])); 	
}
...
$fields = getAll_cond("content_type_attr", "name", array( "id_content" => set_value("tables",$info["tables"])));
...
?>
<option value="<?php echo $v["id_attr"]; ?>" <?php echo $cl; ?>><?php echo $v["name"]; ?></option>
<?
}
?>
</select>
