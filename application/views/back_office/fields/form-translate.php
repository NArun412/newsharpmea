...
$ext = '';
$action = admin_url('manage/submit_translate/'.$this->module.'/'.$this->id.'/'.$this->translate_lang);
echo form_open_multipart($action, $attributes); 
if(isset($id)) {
	echo form_hidden('id', $id);
}
$main_rec = $this->id;
$main_mod = $content_type;
echo form_hidden('module', $main_mod);
echo form_hidden('record', $main_rec);
echo form_hidden('lang', $this->translate_lang);
?>
<section class="tile">
...
if(!$view_only) {
if(isset($id_1) && $id_1 != 0) {

if(get_cell($content_type,'status','id_'.$content_type,$id_1) == 1) {
    echo '<div class="alert alert-success">Published</div>';
}
else {
    echo '<div class="alert alert-danger">Not Published</div>';
}
if(check_approval($content_type,$id_1,2)) {
...
if(check_approval($content_type,$id_1,1)) {
...
//print '22<pre>'; print_r($this->fields); exit;
$this->load->view($this->admin_dir.'/fields/form-fields');
