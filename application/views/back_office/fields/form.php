...
//$render_menu = TRUE;
    ...
    <div class="row">                  
        <div class="col-md-12">
            ...
            if ($this->page_class == 'reviews_status')
                $action .= '?review=1';
...
//print '<pre>'; print_r($info); exit;
                        ...
                        if ($enable_review && user_can_approve()) {
                            ...
                            <div class="form-group">
                                ...
                                if (!isset($info["id_" . $content_type]))
                                    ...
                                    $id_1 = $info["id_" . $content_type];
                                if ($this->user_role == 3 && check_approval($content_type, $id_1, 4))
                                    $selected = 1;
                                if ($this->user_role == 2 && check_approval($content_type, $id_1, 2))
                                    $selected = 1;
                                if ($this->user_role == 1 && check_approval($content_type, $id_1, 2))
                                    $selected = 1;
                                echo form_dropdown($this->page_class == 'reviews_status' ? "approval" : "status", $options, $selected, 'class="form-control"');
                                ?>
                                <?php echo form_error($this->page_class == 'reviews_status' ? "approval" : "status", "<span class='text-lightred'>", "</span>"); ?>
                            </div>
                            <?php } ?>
    ...
    <?php } else { ?>
            ...
            <!-- SEO Details -->
<?php //if(seo_enabled($content_type)) echo render_seo_fields();  ?>
        <?php echo form_close(); ?>
        </div>
        <!--<div class="col-md-6">
        <section class="tile">
