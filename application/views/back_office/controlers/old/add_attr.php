...
<form action="<?= base_url(); ?>back_office/control/submit_attr" method="post" class="middle-forms" enctype="multipart/form-data">
<p class="alert alert-info">Please complete the form below. Mandatory fields marked <em>*</em>
<?
if(isset($id)){
	echo '<input type="hidden" name="id" value="'.$id.'" />';
} else {	
$info=array('name'=>'','type'=>'', 'length' => '', 'thumb_val' => '','display'=>'','foreign_key' => '', 'hint' => '', 'translated' => 0);
}
?>
...
<label class="field-title">Content Type <em>*</em>:</label> <label>
<?
...
<label>
<input type="text" name="hint" value="<?= $info["hint"]; ?>" class="input-xxlarge" />
</label>
<?php $multi_languages = $this->fct->getonecell('settings', 'multi_languages', array('id_settings' => 1));
if($multi_languages == 1){ ?>
<label class="field-title">Translated:</label> 
<label>
<input type="checkbox"  name="translated"  value="1" <? if($info["translated"]==1) echo 'checked="checked"'; ?>  />&nbsp;Yes
</label>
<?php } ?>
