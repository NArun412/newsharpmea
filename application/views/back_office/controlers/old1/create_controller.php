...
$this->form_validation->set_rules("'.str_replace(" ","_",$val["name"]).'", "'.$val["name"].'", "trim");
else
...
if($multi_languages == 1 && $val["translated"] != 1){
$controllers.='
$related_data["'.str_replace(" ","_",$val["name"]).'"] = $_data["'.str_replace(" ","_",$val["name"]).'"];
} else {			
...
$_data["lang"]=$this->fct->getonecell("languages","symbole",array(\'status\' => 1,\'default\' => 1));	
	...
	// Update the traslated rows:
  	if($this->input->post("id_parent") == ""){
	$related_rows = $this->fct->getAll_cond($this->table,"", array("id_parent" => $new_id));
		foreach($related_rows as $related_row){
			$this->db->where("id_'.$table_name.'", $related_row["id_'.$table_name.'"]);
			$this->db->update( $this->table, $related_data );		
		}
	}
	//
	} else {
