...
?>
  <?php if(!empty($icon)) {
	  ...
	  if(!empty($icon['link'])) {
		...
		$link_open = '<a href="'.prep_url($icon['link']).'" '.$target.'>';
	  	$link_close = '</a>';
	  }
	  ?>
  	<div class="ad-icon">
    <?php echo $link_open; ?><img src="<?php echo dynamic_img_url(); ?>popup__icons/icon/500x220/<?php echo $icon['icon']; ?>" alt="<?php echo $icon['icon']; ?>" title="<?php echo $icon['icon']; ?>" /><?php echo $link_close; ?>
    <a class="close-ad-icon" onclick="hideAdIcon()"><!--<i class="fa fa-times-circle-o" aria-hidden="true"></i>--><?php echo lang("close"); ?></a>
    </div>
  <?php }?>
...
<li><a href="<?php echo route_to("contact/inquiry"); ?>" <?php if($this->router->class=='contact' && $this->router->method == 'inquiry'){?>class="active"<?php }?>><?php echo lang("inquiry"); ?></a></li>
    ...
    <div class="products-drop-down">
      <div class="drop-down-tabs">
        <?php foreach($products_menu as $k=>$mn) {
    		  $cl = '';
    	 	  if($k == 0) $cl = 'active';
    		?>
        ...
        <div class="menus" style="display:<?php echo $cl; ?>">
          <?php if(!empty($mn['categories'])){?>
            <?php foreach($mn['categories'] as $k1 => $cat) {?>
              <ul>
                <li><a title="<?php echo $cat['title']; ?>" href="<?php echo route_to("products/index/".$mn['id'].'/'.$cat['id']); ?>"><i class="fa fa-caret-<?php echo $cart_dir; ?>" aria-hidden="true"></i><?php echo $cat['title']; ?></a>
                  <?php if(!empty($cat['lines'])){?>
                  <ul>
                    <?php foreach($cat['lines'] as $k2 => $line) {?>
                    <li><a title="<?php echo $line['title']; ?>" href="<?php echo route_to("products/index/".$mn['id'].'/'.$cat['id'].'/'.$line['id']); ?>"><i class="fa fa-caret-<?php echo $cart_dir; ?>" aria-hidden="true"></i><?php echo $line['title']; ?></a></li>
                    <?php }?>
                  </ul>
                  <?php }?>
                </li>
...
if( (!empty($banner_img) && file_exists("./uploads/banner/".$path."/".$banner_img))){ ?>
      <div>
        <div class="relative span" >
          ...
          <div onclick="redirectBannerDetail('<?php echo $banner['link'];?>')" class="<?php echo ($banner['link'] !='' ? 'cursorPointer centered2' : 'none centered2') ?>" >
