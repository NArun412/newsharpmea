   ...
   <?php $this->load->view("includes/breadcrumbs"); ?>
    <div class="span">
      <div class="product-details-right <?php if(empty($product['image'])) {?>full<?php }?>">
          <h1><?php if($product['product_link']!="") { ?>  <a href="<?php echo site_url().$product['product_link']?>" target="_blank"><?php echo $product['title']; ?></a> <?php }else {echo $product['title']; }?></h1>
        <h4><?php 
        ...
        <?php if(isset($product['brochures']) && !empty($product['brochures'])) {?>	
        	<?php foreach($product['brochures'] as $bro) {?>
            	<div class="country-of-manu no-border">
                    <a onclick="download_file(<?php echo $bro['id']; ?>)"><span class="download-awesome"><i class="fa fa-download" aria-hidden="true"></i></span><?php echo 'download '.$bro['document_type'].'-('.$bro['language'].')'; ?></a>
                </div>
            <?php }?>
        <?php }?>
        </div>
  ...
  <div class="span">
    <div class="may-also-like-slider">
      <?php foreach($similar_products as $product) {
			$link = route_For_Product_Index("products/details/".$product['id']);
		?>
      <div class="item">
