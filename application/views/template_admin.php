...
redirect(base_url().'back_office/home');*/
echo doctype(); ?>
<html class="" lang="en">
<head>
        ...
        <?php echo $_styles;?>
<style>
#popupIframe-shadow { width:100%; height:100%; position:fixed; top:0; left:0; z-index:5000; background-color:rgba(0,0,0,.4); display:none;}
#popupIframe { width:90%; margin:2.5% 5%; position:fixed; top:0; left:0; z-index:5001; background:#fff; height:90%; display:none; }
#popupIframe-loader {width:100%; height:100%;  display:inline-block;overflow:scroll;}
#popupIframe-loader iframe {
	width:100%; height:100%;
}
#popupIframe-close { position:absolute; top:-20px; right:0; z-index:10; color:#fff; cursor:pointer;}
...
<div  id="popupIframe-shadow">
<div id="popupIframe">
 <a class="pull-right" id="popupIframe-close" onClick="closePopUp()">CLOSE X</a>
	<div id="popupIframe-loader">
    </div>
</div>
</div>
        <!-- Splash Modal -->
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button class="btn btn-success btn-ef btn-ef-3 btn-ef-3c" id="add_location"><i class="fa fa-arrow-right"></i> Submit</button>
                        <button class="btn btn-lightred btn-ef btn-ef-4 btn-ef-4c" data-dismiss="modal" id="close_service"><i class="fa fa-arrow-left"></i> Cancel</button>
                        -->
                    </div>
                </div>
            </div>
        ...
        <div id="wrap" class="animsition">
            ...
            ================= CONTENT ===============================
            ===================================================== -->
            <section id="content" <?php if($leftbar == '' || $rightbar == '') {?> style="left:0; top:0"<?php }?>>
            <?php echo $content; ?> 
            </section>
            <!--/ CONTENT -->
        </div>
        ...
        <script src="<?php echo base_url(); ?>assets/js/jquery.min.js"></script>
        		...
        		<script src="<?php echo base_url(); ?>assets/plugins/lou-multi-select-e052211/js/jquery.multi-select.js"></script>
                <script src="<?php echo base_url(); ?>assets/plugins/lou-multi-select-e052211/js/jquery.quicksearch.js"></script>
        <script type="text/javascript">

<?php if($this->router->class == 'install') {?>
<?php if($this->router->method == 'settings') {?>
$(function(){
...
<?php }?>
<?php if($this->router->method == 'next') {?>
...
<?php }?>
<?php if($this->router->class == 'control') {?>
<?php if($this->router->method == 'add_attr' || $this->router->method == 'submit_attr') {?>
$(document).ready(function(){
	var type = $("#type").val();
	if(type != '') {
		$(".opt-type-hide").slideUp("fast");
		$(".opt-type-"+type).slideDown("fast");
	}
	else {
		$(".opt-type-hide").slideUp("fast");
	}
...
<?php }?>
		...
		var csrf_cookie_name = '<?php echo $this->security->get_csrf_hash(); ?>';
		function closePopUp(){
			$('#popupIframe-shadow').fadeOut("fast");
			$('#popupIframe').fadeOut("fast",function(){$('#popupIframe-loader').html('');$('body').removeClass('body-disable');});
		}
		function loadPopup(url)
		{
			$('#popupIframe-shadow').fadeIn("fast");
			$('#popupIframe').fadeIn("fast",function(){
				//$('#popupIframe-loader').html('');
				$('body').addClass('body-disable');
			})
			$.get(url,function(response){
				$('#popupIframe-loader').html('');
				//$('body').addClass('body-disable');
				$('#popupIframe-loader').html(response);
			});
		}
		function loadPopup_iframe(url)
		{
			...
			$('#popupIframe').fadeIn("fast",function(){
				$('#popupIframe-loader').html('');
				$('body').addClass('body-disable');
				$('#popupIframe-loader').html('<iframe src="'+url+'"></iframe>');
			})
		...
		<script src="<?php echo base_url(); ?>assets/js/vendor/magnific-popup/jquery.magnific-popup.min.js"></script>
       ...
       <!-- <script src="<?php echo base_url(); ?>assets/js/vendor/summernote/summernote.min.js"></script>-->
        <script src="<?php echo base_url(); ?>assets/js/jquery.tablednd_0_5.js"></script>
