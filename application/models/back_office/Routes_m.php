...
if($this->table != "seo")
$this->db->join("seo","seo.record = routes.id_routes AND seo.module = 'routes'","LEFT");
$this->db->where($this->table.".id_routes",$id);
$query = $this->db->get();
...
{
	$postdata = $data;
	$fields = $this->db->list_fields("routes");
	$arr = array();
	...
	if(!empty($arr)) {
		...
		if($id == "") {
			...
			if(enable_review() && !user_can_publish())
			$arr["status"] = 0;
			$this->db->insert("routes",$arr);
			$rid = $this->db->insert_id();
			if(enable_review()) {
				$arr["id_routes"] = $rid;
				$this->db->insert("routes_review",$arr);
			}
			if($rid) {
				//if(enable_review() && !user_can_publish())
				$this->fct->record_review("routes",$rid,$default_lang);
				$this->fct->insert_log("create","routes",$rid);
			}
		}
		else {
			$rid = $id;
			$data = $this->get($rid);
			$this->db->where("id_routes",$id)->update("routes".$table_ext,$arr);
			$affected_rows = $this->db->affected_rows();
			if(enable_review()) {
				if(user_can_publish()) {
					$this->db->where("id_routes",$id)->update("routes_review",$arr);
				}
				else {
					if($affected_rows > 0) {
						$this->fct->record_review("routes",$rid,$default_lang);
			...
			if($affected_rows > 0)
			$this->fct->insert_log("update","routes",$rid);
			if(isset($data["status"]) && isset($arr["status"]) && $data["status"] != $arr["status"]) {
				if($data["status"] == 0) {
					$this->fct->change_status("routes",$rid,1);
				}
				else {
					$this->fct->change_status("routes",$rid,0);
				}
			}
		}
		if(enable_review() && isset($postdata["approval"])) {
			$this->fct->change_approval("routes",$rid,$default_lang,$postdata["approval"]);
		}
	}
	return $rid;
}
...
{
	$this->fct->insert_log("delete","routes",$id);
	$data = $this->get($id);
	$this->fct->move_to_trash("routes",$id,$data);
	$this->db->where("id_routes",$id)->delete("routes");
	$this->db->where("translation_id",$id)->delete("routes");
	return true;
}
}