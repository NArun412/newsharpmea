...
function get_div_id_by_category($cat_id)
{
    ...
    {
        if ($this->langue == $this->default_langue) {
            $select = "t.icon, t.title, t.link, t.link_target";
            $cond1  = 't.icon';
        } else {
            $select = "t1.icon, t1.title, t1.link, t1.link_target";
            $cond1  = 't1.icon';
        }
        $this->db->select($select);
    ...
    function get_banners_11($page = 0,$module, $id = 0)
    {
        //$this->db->reset_query();
        if ($this->langue == $this->default_langue)
            $select = "t.id_banner AS id, t.brief, t.image, t.logo, t.title, t.link, t.link_label, t.link_target, t.brief_color, t.hide_title";
...
//			echo $this->db->last_query();exit;
    ...
    function get_one_product($id)
    {
        if (is_numeric($id)) {
            if ($this->langue == $this->default_langue)
                $select = 'p.id_products AS id, p.title, p.image, p.product_type, p.specifications, p.description, c.title AS category_name, d.title AS division_name, c.id_categories AS category_id, d.id_divisions AS division_id, ct.iso_code AS country_code, ct.title AS country_name';
            else
                $select = 'p.id_products AS id, p1.title, p.image, p1.product_type, p1.specifications, p1.description, c1.title AS category_name, d1.title AS division_name, c.id_categories AS category_id, d.id_divisions AS division_id, ct.iso_code AS country_code, ct1.title AS country_name';
            $this->db->select($select);
            $this->db->from("products" . $this->table_ext . " p");
     ...
     /* if ($this->langue == $this->default_langue)
	  ...
	  if(isset($cond['category']) && !empty($cond['category'])) {
		  $this->db->join("products_categories pc","pc.id_products = t.id_products");
                  $this->db->join("categories c","pc.id_categories = c.id_categories");
	  }
    ...
    function filter_support_categories($data = array())
    {
        ...
        if ($this->langue == $this->default_langue)
            $select = 't.id_categories AS id, t.title,t.Code';
        else
